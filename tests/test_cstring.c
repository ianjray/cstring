...
static void test_string_reserve(void)
{
    ...
    assert(-EFAULT == string_reserve(NULL, 8));

    s = string_new();

    assert(-ENOMEM == string_reserve(s, SIZE_MAX));

    memory_shim_fail_at(1);
    assert(-ENOMEM == string_reserve(s, 1));
    memory_shim_reset();
...
static void test_string_c_str_move(void)
{
    ...
    memory_shim_fail_at(1);
    errno = 0;
    assert(NULL == string_c_str_move(s));
    assert(ENOMEM == errno);
    memory_shim_reset();
...
static void test_string_insert_buffer(void)
{
    ...
    assert(-EFAULT == string_insert_buffer(NULL, 1, 3, "foo"));
    ...
    memory_shim_fail_at(1);
    assert(-ENOMEM == string_insert_buffer(s, 0, 1, "a"));
    memory_shim_reset();
    ((struct test_string *)s)->cap = 0;
    ((struct test_string *)s)->len = 0;

    assert(-EFAULT == string_insert_buffer(s, 1, 3, NULL));
    assert(0 == strcmp(string_c_str(s), ""));

    assert(-ERANGE == string_insert_buffer(s, 1, 3, "abcDEF"));

    assert(0 == string_insert_buffer(s, 0, 3, "abcDEF"));
    assert(0 == strcmp(string_c_str(s), "abc"));

    assert(-ENOMEM == string_insert_buffer(s, 1, SIZE_MAX, "XYZ"));
...
static void test_string_insert_c_str(void)
{
    ...
    assert(-EFAULT == string_insert_c_str(NULL, 1, "foo"));

    s = string_new();

    assert(-EFAULT == string_insert_c_str(s, 1, NULL));
    assert(0 == strcmp(string_c_str(s), ""));

    assert(-ERANGE == string_insert_c_str(s, 1, "abc"));
...
static void test_string_insert_fill(void)
{
    ...
    assert(-EFAULT == string_insert_fill(NULL, 0, 3, 'f'));

    s = string_new();

    assert(-ERANGE == string_insert_fill(s, 1, 1, 'o'));
    assert(0 == strcmp(string_c_str(s), ""));
    ...
    memory_shim_fail_at(1);
    assert(-ENOMEM == string_insert_fill(s, 1, 20, 'X'));
    memory_shim_reset();
...
static void test_string_erase(void)
{
    ...
    assert(-EFAULT == string_erase(NULL, 1, 2));

    s = string_new();

    assert(-ERANGE == string_erase(s, 1, 2));
    assert(string_empty(s));
...
static void test_string_push_back(void)
{
    ...
    assert(-EFAULT == string_push_back(NULL, 'f'));
...
static void test_string_pop_back(void)
{
    ...
    assert(-EFAULT == string_pop_back(NULL));

    s = string_new();

    assert(-ERANGE == string_pop_back(s));
...
static void test_string_append_buffer(void)
{
    ...
    assert(-EFAULT == string_append_buffer(NULL, 3, "foo"));

    s = string_new();

    assert(-EFAULT == string_append_buffer(s, 3, NULL));
    cs = s;
    ...
    assert(-ENOMEM == string_append_buffer(s, SIZE_MAX, "XYZ"));

    memory_shim_fail_at(1);
    assert(-ENOMEM == string_append_buffer(s, 20, "12345678901234567890"));
    memory_shim_reset();
...
static void test_string_append_c_str(void)
{
    ...
    assert(-EFAULT == string_append_c_str(NULL, "foo"));

    s = string_new();

    assert(-EFAULT == string_append_c_str(s, NULL));
    assert(0 == strcmp(string_c_str(s), ""));
...
static void test_string_append_fill(void)
{
    ...
    assert(-EFAULT == string_append_fill(NULL, 3, 'f'));
    ...
    memory_shim_fail_at(1);
    assert(-ENOMEM == string_append_fill(s, 20, 'X'));
    memory_shim_reset();
