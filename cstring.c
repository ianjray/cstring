...
char string_at(struct string const *str, size_t pos)
{
    if (pos >= str->len) {
        return 0;
    }
...
struct string *string_substr(struct string const *str, size_t pos, size_t len)
{
...
char string_back(struct string const *str)
{
    if (str->len > 0) {
